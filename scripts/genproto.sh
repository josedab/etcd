...
PROTOC_GEN_GO_BIN=$(tool_get_bin google.golang.org/protobuf/cmd/protoc-gen-go)
PROTOC_GEN_GO_GRPC_BIN=$(tool_get_bin google.golang.org/grpc/cmd/protoc-gen-go-grpc)
...
echo "  - protoc-gen-go:           ${PROTOC_GEN_GO_BIN}"
echo "  - protoc-gen-go-grpc:      ${PROTOC_GEN_GO_GRPC_BIN}"
...
log_callout -e "\\nRunning official protobuf generation..."

for dir in ${DIRS}; do
  run pushd "${dir}"
    run protoc --go_out=. --go-grpc_out=. \
      -I=".:${ETCD_ROOT_DIR}/..:${RAFT_ROOT}:${ETCD_ROOT_DIR}:${GOOGLEAPI_ROOT}" \
      --go_opt=paths=source_relative,Mraftpb/raft.proto=go.etcd.io/raft/v3/raftpb \
      --go-grpc_opt=paths=source_relative,Mraftpb/raft.proto=go.etcd.io/raft/v3/raftpb \
      -I"${GRPC_GATEWAY_ROOT}" \
...
# NOTE: gogo/protobuf has been removed, gw files now use native protobuf v2 messages
# and no longer need patching for v1->v2 conversion.
